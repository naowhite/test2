/*
 * * AI Game Engine Programming p.270
 * 
 * * stateは、singletonでよいのか？複数のobjectで共有してたらまずい。
 *   共有されていた。stateにメンバを持たせて計算すると意図しない動作になる。
 * 　これは作り直しもの。
 *
 * * stateにparentのmachineを持たせたいが、stateの宣言がsingletonのため、とても不便。
 * * stateはobjectごとにnewする必要がある。stateが多すぎたとき、負荷が心配。
 * * statesはunorderedmapで、UpdateでclassIDを返すようにし、map.find(id)で次のstateに切り替える。
 */
